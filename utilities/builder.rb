...
centers = JSON.parse(File.read("#{dirname}/image-centers.json"))
year1 = {}
CSV.foreach("#{dirname}/year1catalog.csv") do |row|
  ...
  year1[fieldname] = object
end

files.each.with_index do |name, index|
  
  wcs = JSON.parse(File.read("#{dirname}/headers/#{name}.fits.json"))
  center = centers[name]
